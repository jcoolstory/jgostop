...
class GoStopMainPanel extends JPanel implements Runnable, MouseListener ,GoFrame
{
	...
	public void DrawUser(Graphics2D g, User user, int x, int y)
	{
		...
		String str = "";
		Font font = new Font("굴림체",Font.BOLD,12);
		
		g.setFont(font);
		g.drawString(user.getScore() + "점", tempx, tempy);
		...
		{
			tempy += 20;
			g.drawString( user.getGo()+ "고", tempx , tempy);
		}
		if (state.SHAKE != 0)
		{
			tempy += 20;
			g.drawString( "흔듦", tempx , tempy);
